...
class REXXD05P1(MethodResource, Resource):
    ...
    def post(self, puzzle):
        ...
        # Run the REXX
        process = Popen(['/prj/repos/aoc2021/rexxes/d5p1.rex',gofile], stdout=PIPE, stderr=PIPE)
...
class REXXD05P2(MethodResource, Resource):
    ...
    def post(self, puzzle):
        ...
        os.system(f"chtag -tc ibm-1047 {gofile}")
        # Run the REXX
        process = Popen(['/prj/repos/aoc2021/rexxes/d5p2.rex',gofile], stdout=PIPE, stderr=PIPE)
        stdout, stderr = process.communicate()
        parts = stdout.decode('utf-8').split('=')
        os.remove(infile)
        os.remove(gofile)
