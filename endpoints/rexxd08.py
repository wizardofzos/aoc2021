...
class REXXD08P1(MethodResource, Resource):
  
    
    @doc(description="<a target='_blank' href='https://adventofcode.com/2021/day/8'>Day 8</a>" +
    ...
    def post(self, puzzle):
        ...
        # Run the REXX
        process = Popen(['/prj/repos/aoc2021/rexxes/d8p1.rex',gofile], stdout=PIPE, stderr=PIPE)
...
class REXXD08P2(MethodResource, Resource):
    ...
    def post(self, puzzle):
        ...
        os.system(f"chtag -tc ibm-1047 {gofile}")
        # Run the REXX
        process = Popen(['/prj/repos/aoc2021/rexxes/d8p2.rex',f'{gofile}'], stdout=PIPE, stderr=PIPE)
        stdout, stderr = process.communicate()
        parts = stdout.decode('utf-8').split('=')
        os.remove(infile)
        os.remove(gofile)
