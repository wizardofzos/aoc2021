...
class REXXD09P1(MethodResource, Resource):
  
    
    @doc(description="<a target='_blank' href='https://adventofcode.com/2021/day/9'>Day 9</a>" +
    ...
    def post(self, puzzle):
        ...
        # Run the REXX
        process = Popen(['/prj/repos/aoc2021/rexxes/d9p1.rex',gofile], stdout=PIPE, stderr=PIPE)
...
class REXXD09P2(MethodResource, Resource):
    ...
    def post(self, puzzle):
        ...
        os.system(f"chtag -tc ibm-1047 {gofile}")
        # Run the REXX
        process = Popen(['/prj/repos/aoc2021/rexxes/d9p2.rex',f'{gofile}'], stdout=PIPE, stderr=PIPE)
        stdout, stderr = process.communicate()
        parts = stdout.decode('utf-8').split('=')
        os.remove(infile)
        os.remove(gofile)
